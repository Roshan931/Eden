...
type DecoratorFunc = (target: any, _key: string | symbol, descriptor: PropertyDescriptor) => PropertyDescriptor 
type Rock = Record<string, unknown>
...
class Scene {
	...
	public static init(script: Script, path: string): DecoratorFunc {
		...
		return (audience: any, _key: string | symbol, descriptor: PropertyDescriptor): PropertyDescriptor => {
	...
	private watchWith(audience: any): any {
		return audience[this.script+getPathsCombined(this.path)] = this.act.bind(this)
	}
